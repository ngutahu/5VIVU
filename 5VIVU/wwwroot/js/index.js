...
function searchFlights() {
    ...
    $('#arrivalAirport option[value="' + departureAirportId + '"]').remove();
            ...
            if (flights.length === 0) {
                ...
                flights.forEach(function (flight) {
                    var row = '<tr>' +
                        ...
                        '<td><button class="btn btn-info" onclick="redirectToFlightDetails(' + flight.flightId + ')">Xem chi tiết</button></td>' +
                        '</tr>';
...
function redirectToFlightDetails(flightId) {
    window.location.href = '/FlightDetails/FlightDetail/' + flightId;
}
